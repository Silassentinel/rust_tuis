...
import { frameAsUntrusted } from "./sanitize.js";
import {
  getLog,
  listTracked,
  startTracking,
...
Error Handling:
  ...
  async ({ session_id, tail_lines }) => {
    try {
      ...
      // The log body is attacker-controlled by construction - it is
      // whatever the tracked process chose to print. `getLog` has already
      // stripped terminal escapes and tagged forged rustlogger markers;
      // this adds the explicit boundary saying the contents are data, not
      // instructions. See sanitize.ts.
      return {
        content: [
          { type: "text" as const, text: header + frameAsUntrusted(result.log) },
        ],
        structuredContent: output,
      };
    } catch (error) {
